...
void sendRF24Message(byte message) {
  RF24NetworkHeader header;
  Serial.println("receiveRF24Message: wysylam wiadomosc");
  network.write(header, &message, sizeof(message));
}
// END:RF23_Methodes----------------------------
...
*/
void sendErrorResponse(IPAddress ip, uint16_t portNumber, char* message, uint16_t errorType, char * errorMessage) {
  Serial.println("!!!!!! sendErrorResponse: !!!!!!!!");
  Serial.print("type: "); Serial.print(errorType); Serial.print(" message: "); Serial.print(errorMessage);
  Serial.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
  builder.init();
  if (parser.parseType(message) == TYPE_CON)
    builder.setType(TYPE_ACK);
